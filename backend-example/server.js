...
app.post("/loginUser", express.json(), async (req, res) => {
  try {
    ...
    if (user && (await bcrypt.compare(password, user.password))) {
      ...
      // Send JSON Web Token to valid user, implicitly status 200
...
app.delete("/deleteNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      // If note is not found, report an error
      if (!data) {
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    if (!ObjectId.isValid(noteId) || !title || !content) {
      return res.status(400).json({ error: "Invalid note ID or the body params." });
    }
